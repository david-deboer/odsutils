...
# Data file options
ap.add_argument('-f', '--data-file', dest='data_file', help="Name of data file to read", default=None)
ap.add_argument('--sep', help="Separator for the data file - 'auto' tries to determine from first line", default='auto')
ap.add_argument('--replace-char', dest='replace_char', help="Replace character in data file column header names", default=None)
ap.add_argument('--header-map', dest='header_map', help="Name of json file that maps data file headers to ODS keys", default=None)
# Types of "culling/updating"
ap.add_argument('-u', '--update-el', dest='update_el', help="Update flag for above elevation limit)", action='store_true')
ap.add_argument('--el-lim', dest='el_lim', help="Elevation limit in deg", type=float, default=10.0)
ap.add_argument('--dt-sec', dest='dt_sec', help="Time step in seconds to do elevation check.", type=float, default=120.)
...
ap.add_argument('-t', '--time-cull', dest='time_cull', help="Cull existing ods file on time - 'now' or isoformat", default=False)
ap.add_argument('--cull-by', dest='cull_by', help="Type of time cull", choices=['stale', 'inactive'], default='stale')
ap.add_argument('-i', '--invalid-cull', dest='invalid_cull', help="Cull ods of invalid entries", action='store_true')
# Output
...
ap.add_argument('-s', '--std-show', dest='std_show', help="Show the terms of an ODS record", action='store_true')
# ODS fields
